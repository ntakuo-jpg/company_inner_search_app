...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        st.markdown(
            '<div style="background-color: #e8f5e9; padding: 15px; border-radius: 8px; color: #2e7d32;">'
            'こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。サイドバーで利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。'
            '</div>',
            unsafe_allow_html=True
        )
        st.markdown(
            '<div style="background-color: #fff9e6; padding: 15px; border-radius: 8px; color: #000000; margin-top: 10px;">'
            '⚠️ 具体的な入力をすると期待通りの回答が得やすいです。'
            '</div>',
            unsafe_allow_html=True
        )
